...
import { createProperty, updateProperty, useGetProperty, useGetProperties } from '../../../actions/property';

import type { IPropertyItem } from '../../../types/property';
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    const promise = await (async () => {
      ...
      if (currentProperty?.id) {
        ...
        response = await updateProperty({ ...data, updatedby: shortUser, changes }, currentProperty.id);
      } else {
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      <Stack direction="row" justifyContent="flex-end" gap={4} mt={2} mb={5}>
        ...
        {isEdit && (
          ...
          >
            {currentProperty ? 'Actualizar' : 'Crear'} cliente
          </Button>
        )}
      </Stack>
    </Form>
